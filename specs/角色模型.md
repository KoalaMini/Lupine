...
每个 Agent 挂载的 Skill 直接影响其工作效果。以下是经 Lupine 验证的推荐配置：

| Skill | 推荐对象 | 来源 | 适用场景 |
|-------|---------|------|---------|
| **brainstorming** | Lupine（主Agent） | 外部 `obra/superpowers` | 需求探讨阶段，发散→收敛 |
| **lupine-diagram** | Lupine（主Agent） | 自研（builtin） | 架构展示、方案对比、审查报告可视化 |
| **impeccable** | 评估器（Evaluator） | 外部 `lupine-ai/skills` | 代码质量与规范审查 |

> 规划器、执行器、探索器当前无特定推荐 Skill。

**Skill 加载原则**：
- 项目级 Skill 跟随仓库（`.opencode/skills/` 或 `.claude/skills/`）
- Lupine 代装推荐 Skill（Init 时自动安装）
- 非推荐 Skill 需经用户确认纳入（效果变差风险告知）
...
| 产品版本 | 变更内容 | 状态 |
|---------|---------|------|
| v0.6 | 新增 Skill 推荐分配（各 Agent 挂载的推荐 Skill 清单、Skill 加载原则） | in-progress |
